...
describe('citrixStorage', () => {
  beforeEach(() => {
    ...
    if ((globalThis as unknown as { window?: Record<string, unknown> }).window) {
      delete (globalThis as unknown as { window: Record<string, unknown> }).window.showDirectoryPicker;
    }
  });
  ...
  it('resets singleton instance cleanly with _resetCitrixStorageAdapterForTesting', () => {
    ...
    (adapter1 as unknown as Record<string, unknown>).testMarker = 42;

    _resetCitrixStorageAdapterForTesting();
    const adapter2 = getCitrixStorageAdapter();
    expect((adapter2 as unknown as Record<string, unknown>).testMarker).toBeUndefined();
